...
export default function PlClassesScreen() {
  ...
  const filtered = classes.filter(c => (c.classCode || c.courseCode || c.venue || '').toLowerCase().includes(search.toLowerCase()));
  if (loading || !faculty) return <ActivityIndicator style={styles.center} size="large" color="#007AFF" />;

  return (
    <View style={styles.container}>
      <Text style={styles.header}>All Classes & Venues ({faculty})</Text>
      <SearchBar placeholder="Search class, course, venue..." value={search} onChangeText={setSearch} />
      <FlatList data={filtered} keyExtractor={item => item.id} contentContainerStyle={{ padding: 12 }}
        ...
        )} ListEmptyComponent={<Text style={styles.empty}>No classes created yet.</Text>} />
    </View>
  );
}
